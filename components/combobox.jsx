...
const lookStyles = {
  ...
  secondary: {
    ...
    selectedItemIcon: { active: 'h-5 w-5 text-white', inactive: 'h-5 w-5 text-orange-500' },
  },
}
              ...
              {filteredData.length === 0 && query !== '' ? (
                ...
                filteredData.map((item) => (
                  ...
                  >
                    {({ selected, active }) => (
                      <>
                        <ItemDisplay selected={selected} active={active} item={item} />
                        {selected && (
                          <span className="absolute inset-y-0 left-0 grid w-10 place-items-center">
                            <CheckIcon
                              className={
                                active
                                  ? lookStyles[look].selectedItemIcon.active
                                  : lookStyles[look].selectedItemIcon.inactive
                              }
                            />
                          </span>
                        )}
